...
classes=10
epochs=100
savedir = os.path.join(os.getcwd(),'savedConvNets')
name = 'keras_cifar10_cnn_trained.h5'
...
model.compile(loss='categorical_crossentropy',optimizer='rmsprop',metrics=['accuracy','binary_accuracy',precision,recall,fbeta_score])
model.fit(Xtrain,Ytrain,batch_size=batch,epochs=epochs,verbose=2,validation_split=0.2)
if not os.path.isdir(savedir):
	os.makedirs(savedir)
path = os.path.join(savedir,name)
model.save_weights(path)
print('Saved the model at {}'.format(path))		
score = model.evaluate(Xtest, Ytest, verbose=1)
print('Test loss:', score[0])
print('Test accuracy:', score[1])
print('Binary(One-vs-All) accuracy:', score[2])
print('Test Precision:', score[3])
print('Test Recall:', score[4])
print('Test F-measure:', score[5])